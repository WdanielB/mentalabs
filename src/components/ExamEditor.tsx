...
const Q_TYPES: { id: QType; label: string; icon: React.ElementType; desc: string; color: string }[] = [
 { id: "likert", label: "Likert", icon: BarChart2, desc: "Escala de puntajes", color: "text-blue-600 bg-blue-50 " },
 { id: "yesno", label: "Sí / No", icon: CheckSquare, desc: "Respuesta binaria", color: "text-green-600 bg-green-50 " },
 { id: "vas", label: "Escala 0-10", icon: Sliders, desc: "Intensidad visual", color: "text-purple-600 bg-purple-50 " },
 { id: "single_choice", label: "Opción Única", icon: CircleDot, desc: "Lista de opciones", color: "text-orange-600 bg-orange-50 " },
 { id: "free_text", label: "Texto Libre", icon: AlignLeft, desc: "Respuesta abierta", color: "text-slate-600 bg-slate-100 " },
 { id: "interactive_game", label: "Juego", icon: Gamepad2, desc: "Juego interactivo JSON", color: "text-pink-600 bg-pink-50 " },
];
...
export default function ExamEditor({ examId, backHref }: { examId: string; backHref: string }) {
 ...
 {saveError && (
 <span className="text-xs text-red-500 font-semibold bg-red-50 px-3 py-1.5 rounded-lg border border-red-200 max-w-xs truncate">
 ⚠️ {saveError}
 </span>
 )}
 ...
 className="w-full bg-transparent resize-none outline-none text-sm text-slate-700 placeholder:text-slate-400" />
 </div>
 ...
 Preguntas
 <span className="ml-2 text-sm font-semibold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{questions.length}</span>
 </h2>
 ...
 <button onClick={() => moveQ(qi, "up")} disabled={qi === 0} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 disabled:opacity-30 transition-colors"><ChevronUp className="h-4 w-4" /></button>
 <button onClick={() => moveQ(qi, "down")} disabled={qi === questions.length - 1} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 disabled:opacity-30 transition-colors"><ChevronDown className="h-4 w-4" /></button>
 <button onClick={() => deleteQ(qi)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors"><Trash2 className="h-4 w-4" /></button>
 </div>
 </div>
 ...
 <div key={side} className={`p-4 rounded-xl border-2 ${side === "yes" ? "border-green-200 bg-green-50 " : "border-red-200 bg-red-50 "}`}>
 <p className={`text-sm font-bold mb-3 ${side === "yes" ? "text-green-700 " : "text-red-600 "}`}>{side === "yes" ? "✅ Respuesta Sí" : "❌ Respuesta No"}</p>
 <div className="space-y-2">
 <div>
 ...
 className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-slate-200 hover:border-pink-400 hover:bg-pink-50 hover:text-pink-600 transition-all">
 {tpl.label}
 </button>
 ))}
 </div>
 ...
 <div className="flex items-center gap-2 p-3 bg-pink-50 rounded-xl border border-pink-200 text-xs text-pink-700 ">
