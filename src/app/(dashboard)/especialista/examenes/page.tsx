...
export default function EspecialistaExamenesPage() {
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      {/* Info banner */}
      <div className="mb-6 flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-200 dark:border-blue-800">
      ...
      {loading && (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-20 rounded-2xl bg-white dark:bg-[#1a2432] border border-slate-200 dark:border-slate-800 animate-pulse" />
          ))}
        </div>
      )}
      ...
      <div className="space-y-3">
        {filtered.map(exam => {
          ...
          return (
            <div key={exam.id} className="bg-white dark:bg-[#1a2432] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              <button onClick={() => toggle(exam.id)}
                ...
                <span className="flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400 shrink-0">
              ...
              {isOpen && (
                <div className="px-5 pb-5 border-t border-slate-100 dark:border-slate-800">
                  {exam.description && (
                    <p className="text-sm text-slate-600 dark:text-slate-300 mt-4 mb-4 leading-relaxed">{exam.description}</p>
                  )}
