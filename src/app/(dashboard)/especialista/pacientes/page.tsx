    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="bg-white dark:bg-[#1a2432] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        {loading && Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-4">
        ...
        {filtered.map((p) => {
          ...
          return (
            <div key={p.id} className="grid grid-cols-1 md:grid-cols-5 gap-2 md:gap-4 px-6 py-4 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors items-center">
              <div className="col-span-2 flex items-center gap-3">
                ...
                {p.pendingExams > 0 && (
                  <span className="flex items-center gap-1 text-xs font-bold text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 px-2 py-0.5 rounded-full shrink-0">
      ...
      {assignTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white dark:bg-[#1a2432] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            {/* Modal header */}
            ...
            <div className="p-4 max-h-80 overflow-y-auto">
              ...
              {!loadingExams && publishedExams.length === 0 && (
                <div className="text-center py-10">
                  <ClipboardList className="h-10 w-10 mx-auto mb-3 text-slate-300 dark:text-slate-600" />
              ...
              {assignedFlash && (
                <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800">
                  ...
                  <div>
                    <p className="font-bold text-sm text-green-700 dark:text-green-400">¡Examen asignado!</p>
                    <p className="text-xs text-green-600 dark:text-green-500">El paciente lo verá en su dashboard.</p>
                  </div>
                </div>
              )}
              ...
              {!assignedFlash && publishedExams.map((exam) => (
                ...
                >
                  <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                    selectedExam === exam.id ? "bg-[#136dec] text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500"
                  }`}>
