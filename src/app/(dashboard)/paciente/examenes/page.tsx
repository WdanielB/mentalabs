...
const STATUS_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string; bg: string }> = {
  pending:     { label: "Pendiente",   icon: Clock,        color: "text-yellow-600", bg: "bg-yellow-50 dark:bg-yellow-900/20" },
  in_progress: { label: "En Progreso", icon: PlayCircle,   color: "text-blue-600",   bg: "bg-blue-50 dark:bg-blue-900/20" },
  completed:   { label: "Completado",  icon: CheckCircle2, color: "text-[#0bda5e]",  bg: "bg-green-50 dark:bg-green-900/20" },
};
    ...
    <div className="p-6 lg:p-8">
      ...
      {loading && (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-28 rounded-2xl bg-white dark:bg-[#1a2432] border border-slate-200 dark:border-slate-800 animate-pulse" />
          ))}
        </div>
      )}
      ...
      {!loading && attempts.length === 0 && (
        <div className="text-center py-20 bg-white dark:bg-[#1a2432] rounded-2xl border border-slate-200 dark:border-slate-800">
          <ClipboardList className="h-14 w-14 mx-auto mb-4 text-slate-300 dark:text-slate-600" />
          <p className="font-bold text-slate-700 dark:text-slate-300 text-lg">Sin exámenes asignados</p>
        ...
        <section className="mb-8">
          ...
          <div className="space-y-4">
            {pending.map((a) => {
              ...
              return (
                <div key={a.id} className="flex items-center gap-5 p-5 bg-white dark:bg-[#1a2432] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all">
      ...
      {!loading && completed.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {completed.map((a) => (
              <div key={a.id} className="flex items-center gap-5 p-5 bg-white dark:bg-[#1a2432] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm opacity-80">
