...
function ExamFormModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-white dark:bg-[#1a2432] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <h2 className="font-bold text-base">{initial ? "Editar Examen" : "Nuevo Examen"}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"><X className="h-5 w-5" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            ...
            <input type="text" autoFocus value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()}
              ...
              className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#111822] text-sm font-semibold outline-none focus:ring-2 focus:ring-[#136dec] focus:border-transparent transition-all" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={battery} onChange={e => setBattery(e.target.value)}
                ...
                className="w-full h-10 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#111822] text-sm outline-none focus:ring-2 focus:ring-[#136dec] focus:border-transparent transition-all" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Tipo de Diagnóstico</label>
              <select value={dtype} onChange={e => setDtype(e.target.value)}
                className="w-full h-10 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#111822] text-sm outline-none focus:ring-2 focus:ring-[#136dec] transition-all">
          ...
          <div>
            ...
            <textarea rows={3} value={desc} onChange={e => setDesc(e.target.value)}
              ...
              className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#111822] text-sm resize-none outline-none focus:ring-2 focus:ring-[#136dec] focus:border-transparent transition-all" />
          </div>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">Cancelar</button>
      ...
      <main className="flex-1 lg:ml-64 p-6 lg:p-8" ref={menuRef}>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar examen..."
              className="h-9 pl-9 pr-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1a2432] text-sm focus:ring-2 focus:ring-[#136dec] focus:border-transparent outline-none transition-all w-48" />
          </div>
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-xl p-1">
            {STATUS_FILTER.map(s => (
              <button key={s} onClick={() => setStatus(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${status === s ? "bg-white dark:bg-[#1a2432] shadow-sm text-[#136dec]" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}>
                {s}
              </button>
            ))}
          </div>
          <select value={dtFilter} onChange={e => setDtFilter(e.target.value)}
            className="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1a2432] text-sm outline-none focus:ring-2 focus:ring-[#136dec] transition-all">
        ...
        {loading && (
          <div className="space-y-4">
            {Array.from({ length: 2 }).map((_, i) => <div key={i} className="h-32 rounded-2xl bg-white dark:bg-[#1a2432] border border-slate-200 dark:border-slate-800 animate-pulse" />)}
          </div>
        )}
        ...
        {Object.entries(grouped).map(([battery, items]) => (
          <div key={battery} className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map(exam => (
                <div key={exam.id} className="group relative bg-white dark:bg-[#1a2432] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 transition-all overflow-hidden">
                  <div className={`h-1.5 w-full ${exam.is_published ? "bg-gradient-to-r from-[#0bda5e] to-[#136dec]" : "bg-slate-200 dark:bg-slate-700"}`} />
                  <div className="p-5">
                    {/* Badges */}
                    <div className="flex flex-wrap gap-2 mb-3">
                      {exam.meta.diagnosis_type && (
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-blue-100 text-[#136dec] dark:bg-blue-900/20">{exam.meta.diagnosis_type}</span>
                      )}
                      <span className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${exam.is_published ? "bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400" : "bg-slate-100 text-slate-500 dark:bg-slate-800"}`}>
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <div className="relative shrink-0">
                        <button onClick={() => setMenuOpen(menuOpen === exam.id ? null : exam.id)}
                          className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 opacity-0 group-hover:opacity-100 transition-all">
                          <MoreVertical className="h-4 w-4" />
                        </button>
                        {menuOpen === exam.id && (
                          <div className="absolute right-0 top-8 z-20 w-48 bg-white dark:bg-[#1a2432] rounded-xl border border-slate-200 dark:border-slate-800 shadow-lg py-1">
                            <Link href={`/admin/banco-pruebas/${exam.id}`} className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                            ...
                            <button onClick={() => { setModal({ id: exam.id, title: exam.title, meta: exam.meta }); setMenuOpen(null); }}
                              className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
