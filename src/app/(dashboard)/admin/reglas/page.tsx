...
const diagColor = (t: string) => DIAGNOSIS_COLORS[t] ?? "bg-slate-100 text-slate-600 ";
...
function RuleForm({
 ...
 <button onClick={onCancel} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 transition-colors">Cancelar</button>
 </div>
 </div>
 );
}
...
export default function ReglasPage() {
 ...
 if (pct < 0.66) return "bg-yellow-100 text-yellow-700 ";
 ...
 <div key={i} className="mx-3 my-2 h-14 rounded-xl bg-slate-100 animate-pulse" />
 ))}
 ...
 <ShieldCheck className="h-14 w-14 mx-auto mb-4 text-slate-300 " />
