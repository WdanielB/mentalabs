        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SOLUTIONS.map((s) => (
            <div key={s.id} className={`group rounded-3xl border ${s.border} ${s.bg} p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-1`}>
              ...
              <h3 className="font-display text-xl font-700 text-slate-900 dark:text-white mb-3">{s.full}</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">{s.desc}</p>
              <ul className="space-y-2 mb-6">
                {s.tools.map((t) => (
                  <li key={t} className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
