...
export default function RegisterPage() {
 ...
 <h2 className="text-3xl font-bold text-slate-900 mb-2">Bienvenido a MentaLabs</h2>
 <p className="text-slate-600 mb-8">Para comenzar, dinos quién eres para personalizar tu experiencia.</p>

 {/* Role Selection */}
 <div className="mb-8">
 <label className="block text-sm font-medium text-slate-900 mb-4">Yo soy...</label>
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
 <button 
 onClick={() => setRole("padre")}
 className={`p-4 border rounded-xl flex flex-col items-center gap-3 transition-colors ${role === 'padre' ? 'border-[#136dec] bg-blue-50 text-[#136dec]' : 'border-slate-200 text-slate-600 hover:bg-slate-50 '}`}>
 <Users className="h-6 w-6" />
 <span className="font-medium text-sm">Padre/Madre</span>
 </button>
 <button 
 onClick={() => setRole("paciente")}
 className={`p-4 border rounded-xl flex flex-col items-center gap-3 transition-colors ${role === 'paciente' ? 'border-[#136dec] bg-blue-50 text-[#136dec]' : 'border-slate-200 text-slate-600 hover:bg-slate-50 '}`}>
 <User className="h-6 w-6" />
 <span className="font-medium text-sm">Paciente Adulto</span>
 </button>
 <button 
 onClick={() => setRole("especialista")}
 className={`p-4 border rounded-xl flex flex-col items-center gap-3 transition-colors ${role === 'especialista' ? 'border-[#136dec] bg-blue-50 text-[#136dec]' : 'border-slate-200 text-slate-600 hover:bg-slate-50 '}`}>
 ...
 <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">Información Personal</h3>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 <label className="block text-sm font-medium text-slate-700 mb-1">Nombre</label>
 <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej. Juan" required className="w-full rounded-lg border-slate-200 bg-white p-3 text-sm focus:border-[#136dec] focus:ring-[#136dec]" />
 </div>
 <div>
 <label className="block text-sm font-medium text-slate-700 mb-1">Apellido</label>
 <input type="text" value={apellido} onChange={(e) => setApellido(e.target.value)} placeholder="Ej. Pérez" required className="w-full rounded-lg border-slate-200 bg-white p-3 text-sm focus:border-[#136dec] focus:ring-[#136dec]" />
 </div>
 </div>

 <div>
 <label className="block text-sm font-medium text-slate-700 mb-1">Correo Electrónico</label>
 <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nombre@ejemplo.com" required className="w-full rounded-lg border-slate-200 bg-white p-3 text-sm focus:border-[#136dec] focus:ring-[#136dec]" />
 </div>
 ...
 <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mínimo 8 caracteres" required className="w-full rounded-lg border-slate-200 bg-white py-3 pl-10 pr-3 text-sm focus:border-[#136dec] focus:ring-[#136dec]" />
 </div>
 </div>
 ...
 <Link href="/" className="flex-1 flex justify-center items-center text-center py-3 rounded-lg font-semibold border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors">
 Atrás
 </Link>
