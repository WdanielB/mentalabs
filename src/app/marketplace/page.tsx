...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1,2,3,4,5].map(n => (
        <Star key={n} className={`h-3.5 w-3.5 ${n <= Math.floor(rating) ? "text-yellow-400 fill-yellow-400" : "text-slate-200 dark:text-slate-700"}`} />
      ))}
    </div>
  );
}
...
function BookModal({ specialist, patientId, onClose }: { specialist: SpecialistCard; patientId: string | null; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white dark:bg-[#131f2e] rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-slate-700">
          <div>
            <h2 className="font-display font-700 text-slate-900 dark:text-white">{specialist.full_name}</h2>
        ...
        <div className="p-6">
          {success ? (
            <div className="text-center py-6">
              ...
              <h3 className="font-display font-700 text-xl text-slate-900 dark:text-white mb-2">¡Cita agendada!</h3>
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={onClose} className="px-5 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-semibold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Cerrar</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 gap-2 mb-6">
                {SLOTS.map(t => (
                  <button key={t} onClick={() => setSelected(t)}
                    className={`py-2.5 px-3 rounded-xl text-sm font-semibold border-2 transition-all ${selected === t ? "border-[#136dec] bg-blue-50 dark:bg-[#136dec]/10 text-[#136dec]" : "border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-600"}`}>
                    {t}
                  </button>
                ))}
              </div>
              {err && <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-xl text-sm text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800">{err}</div>}
              <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700">
                <div>
                  <p className="text-xs text-slate-500">Sesión 60 min</p>
                  <p className="font-display font-800 text-2xl text-slate-900 dark:text-white">${specialist.hourly_rate}</p>
                </div>
...
export default function MarketplacePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a1018] text-slate-900 dark:text-slate-100">
      <nav className="sticky top-0 z-40 w-full border-b border-slate-200/80 bg-white/95 backdrop-blur-xl dark:border-slate-800/80 dark:bg-[#0d1520]/95">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 lg:px-8">
          ...
          <Link href="/login" className="text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-[#136dec] transition-colors">Iniciar sesión</Link>
        </div>
      </nav>

      <div className="bg-white dark:bg-[#0d1520] border-b border-slate-200 dark:border-slate-800">
        <div className="mx-auto max-w-7xl px-6 lg:px-8 py-12">
          <h1 className="font-display text-4xl lg:text-5xl font-800 tracking-tight text-slate-900 dark:text-white mb-3">Encuentra a tu especialista</h1>
          <p className="text-slate-500 dark:text-slate-400 mb-8">Especialistas certificados en neurodivergencias, listos para evaluarte.</p>
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Nombre o especialidad..."
                className="w-full h-11 pl-11 pr-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#131f2e] text-sm focus:ring-2 focus:ring-[#136dec] focus:border-transparent outline-none transition-all" />
            </div>
            <div className="flex items-center gap-2 overflow-x-auto">
              {SPECIALTIES.map(s => (
                <button key={s} onClick={() => setSpecialty(s)}
                  className={`shrink-0 px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${specialty === s ? "bg-[#136dec] text-white border-[#136dec]" : "bg-white dark:bg-[#131f2e] text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-[#136dec]/50"}`}>
                  {s}
                </button>
              ))}
            </div>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 py-10">
        ...
        {loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-64 rounded-3xl bg-white dark:bg-[#131f2e] border border-slate-200 dark:border-slate-700 animate-pulse" />
            ))}
          </div>
        )}

        {!loading && all.length === 0 && (
          <div className="text-center p-12 bg-blue-50 dark:bg-blue-900/10 rounded-3xl border border-blue-200 dark:border-blue-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(spec => (
            <div key={spec.id} className="group bg-white dark:bg-[#131f2e] rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              <div className="relative h-20 bg-gradient-to-br from-[#136dec] to-[#0043b5]">
                <div className="absolute -bottom-8 left-6">
                  <div className="h-16 w-16 rounded-2xl border-4 border-white dark:border-[#131f2e] bg-gradient-to-tr from-[#0bda5e] to-[#136dec] flex items-center justify-center text-white font-display font-800 text-2xl shadow-lg">
              ...
              <div className="pt-10 px-6 pb-6">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="font-display font-700 text-slate-900 dark:text-white">{spec.full_name}</h3>
                    <p className="text-sm text-[#136dec] font-semibold">{spec.specialty}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-display font-700 text-slate-900 dark:text-white">${spec.hourly_rate}</p>
                ...
                {spec.bio && <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-5 line-clamp-2">{spec.bio}</p>}
